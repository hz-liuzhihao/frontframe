...
export function SimpleFormWrapper(Wrapper, args) {
  return function (props) {
    ...
    const { decorate, wrapperProps = {}, rules = [], initialValue, children, required, requiredMessage, label, style, className, whitespace = false, fieldKey, ...resetProps } = props;
    const innerRules = [];
    if (whitespace) {
      innerRules.push({
        whitespace: true,
    ...
    return <Form.Item {...resetProps} style={style} className={className} required={required} name={decorate} fieldKey={fieldKey} initialValue={initialValue} validateFirst label={label} rules={[
      {
        required: required,
        message: requiredMessage || `${label || ''}不能为空`,
      },
...
/**
  * 输入表单组件
  */
export const FormInput = SimpleFormWrapper(Input);
 ...
 * 下拉选择表单组件
 */
export const FormSelect = SimpleFormWrapper(Select);

/**
 * 开关表单组件
 */
export const FormSwitch = SimpleFormWrapper(InputSwitch);

/**
 * 单选组件
 */
export const FormRadio = SimpleFormWrapper(Radio.Group);
